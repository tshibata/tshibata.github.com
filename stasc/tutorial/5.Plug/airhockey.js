...
function light(g, p) {
	for (var i = 0; i < 7; i++) {
		if (i < p) {
			g.appendChild(On(35 * i));
		} else {
			g.appendChild(Off(35 * i));
		}
	}
}
var svg;
			...
			if (tLeft <= t && tLeft < tRight && tLeft < tTop && tLeft < tBottom && tLeft < tYou && tLeft < tFoe) {
				...
				if (100 <= puckX && puckX < 220) {
					...
					if (got < 7) {
						...
						svg.Overlay = Score(got + " - " + lost);
					} else {
						svg.Overlay = End(got + " - " + lost);
					}
					light(svg.Overlay.Lost, lost);
					light(svg.Overlay.Got, got);
					//svg.removeChild(puck);
